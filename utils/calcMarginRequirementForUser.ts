 ...
 */
export function calcMarginRequirementForUser(context: Context, userAccountAddressInput?: PublicKey): Promise<[number, number]> {
        ...
        try {
            ...
            if (i > -1) {
                if (side === OrderSide.Ask) {
                    userNetPositions[i] -= size * 10 ** USDC_DECIMALS;
                }
                ...
                if (side === OrderSide.Ask) {
                    ...
                    positions.push({
                        ...
                        longQty: 0,
                        shortQty: size * 10 ** USDC_DECIMALS,
                    });
                    userNetPositions.push(-size * 10 ** USDC_DECIMALS);
                } else {
                    // @ts-ignore
                    positions.push({
                        instrument: marketAccountInfo.instrument,
                        longQty: size * 10 ** USDC_DECIMALS,
                    });
                    userNetPositions.push(size * 10 ** USDC_DECIMALS);
                }
            }
...
export function isMarginSufficientForNewOrder(
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            let [marginRequirement, netOptionValue] = await preCalcMarginForNewOrder(context, userAccountAddress, marketAddress, side, maxCoinQty);
...
async function calcMarginForOneAsset(
    context: Context,
    asset: number,
    usdcSpot: number,
    strikeRaw: number[],
    isCallRaw: number[],
    userPositionsRaw: number[],
