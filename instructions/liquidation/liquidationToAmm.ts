...
export default function liquidationToAmm(context: Context, userAccountAddress: PublicKey, marketAddress: PublicKey): Promise<InstructionResult<TransactionSignature>> {
            ...
            .then(([exchangeAddress, _]) => {
                context.program.account.optifiMarket
                    ...
                    .then((marketRes) => {
                        ...
                        findLiquidationState(context, userAccountAddress)
                            .then(([liquidationStateAddress, _]) => {
                                getSerumMarket(context, market.serumMarket)
                                    .then((serumMarket) => {
                                        context.program.account.userAccount
                                            .fetch(userAccountAddress)
                                            .then((userAccount) => {
                                                findOrCreateAssociatedTokenAccount(context, market.instrumentLongSplToken, userAccountAddress)
                                                    .then((longSPLTokenVault) => {
                                                        findOrCreateAssociatedTokenAccount(context, market.instrumentShortSplToken, userAccountAddress)
                                                            .then(async (shortSPLTokenVault) => {
                                                                let chainRes = await context.program.account.chain.fetch(marketRes.instrument);
                                                                // @ts-ignore
                                                                ...
                                                                let [ammLongTokenVault] = await findAssociatedTokenAccount(
                                                                    context,
                                                                    market.instrumentLongSplToken,
                                                                    ammLiquidityAuth
                                                                );

                                                                let [ammShortTokenVault] = await findAssociatedTokenAccount(
                                                                    context,
                                                                ...
                                                                let updateMarginStressInx = await marginStress(context, ammForLiquidation[0][0].asset);
                                                                ...
                                                                context.program.rpc
                                                                    .liquidationToAmm({
                                                                        accounts: {
                                                                            ...
                                                                            instrumentLongSplTokenMint: serumMarket.decoded.baseMint,
